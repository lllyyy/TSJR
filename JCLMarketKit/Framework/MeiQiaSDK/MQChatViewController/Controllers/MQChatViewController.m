...
@property (nonatomic, strong) UIButton *bt;//只用来显示UIButton *bt
@end
...
- (void)addTestBt{
   ...
   self.bt = [UIButton buttonWithType:UIButtonTypeCustom];
    self.bt.frame = CGRectMake(15, 20, 40, 40);
//    bt.backgroundColor = [UIColor redColor];
    [self.bt setImage: [MQAssetUtil backArrow] forState:0];
//    [MQAssetUtil backArrow]
    UIWindow *aWindow = [[UIApplication sharedApplication] delegate].window;
    [aWindow addSubview:self.bt];
    
    [self.bt addTarget:self action:@selector(closeSocketlalala) forControlEvents:UIControlEventTouchUpInside];
}
